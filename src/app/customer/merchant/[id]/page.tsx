...
import { ArrowLeft, MapPin, Store, Clock, Info, PackageOpen } from "lucide-react";
...
export default async function MerchantDetailPage(props: {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 relative">
      <Link href="/customer/home" className="inline-flex items-center text-sm font-bold text-neutral-dark/60 hover:text-primary transition-colors bg-white/50 backdrop-blur-md px-4 py-2 rounded-xl border border-white shadow-sm mb-2">
      ...
      <div className="bg-white rounded-[2.5rem] shadow-xl shadow-neutral-light/20 border border-white/60 overflow-hidden relative">
        ...
        <div className="h-48 md:h-64 relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-accent/90 via-primary to-[#0f172a]" />
        ...
        <div className="px-4 md:px-10 pb-10">
          ...
          <div className="relative flex justify-between items-end -mt-12 md:-mt-20 mb-5 z-10">
            <div className="h-32 w-32 bg-white rounded-3xl shadow-xl flex items-center justify-center border-[6px] border-white relative group overflow-hidden flex-shrink-0">
            ...
            <div className={`px-4 md:px-5 py-2 md:py-3 rounded-xl text-xs md:text-sm font-bold shadow-lg flex items-center gap-2 leading-normal mb-2 ${
              merchant.isOpen 
              ...
              <div className={`w-2 h-2 rounded-full flex-shrink-0 ${merchant.isOpen ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'}`} />
              <span className="whitespace-nowrap">{merchant.isOpen ? 'Buka Sekarang' : 'Sedang Tutup'}</span>
            </div>
          </div>
          
          {/* Info Toko (Aman di area putih, tidak menimpa banner) */}
          <div className="mb-8">
            <h1 className="text-2xl md:text-4xl font-extrabold text-primary tracking-tight mb-2 leading-normal">{merchant.storeName}</h1>
            <p className="text-sm font-bold text-neutral-dark/50 uppercase tracking-widest leading-relaxed">Dikelola oleh {merchant.user.name}</p>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 md:gap-8 text-sm font-medium mb-8 bg-neutral-50/80 p-5 rounded-2xl border border-neutral-light/50">
            <div className="flex items-start text-neutral-dark/80">
              ...
              <span className="leading-relaxed">
                <span className="font-bold text-primary">{merchant.area}</span> — {merchant.address}
              </span>
            </div>
            ...
            <div className="flex items-center text-neutral-dark/80 whitespace-nowrap">
              ...
              Ongkir: <span className="font-bold text-primary ml-1">Rp {merchant.deliveryFee.toLocaleString("id-ID")}</span>
            </div>
          </div>
          ...
          <div className="border-t border-neutral-light/50 pt-10">
            ...
            {merchant.products.length === 0 ? (
              <div className="bg-neutral-50/50 rounded-2xl p-10 text-center border border-dashed border-neutral-light/80">
                ...
                <p className="text-sm text-neutral-dark/60 mt-1">Merchant ini belum menambahkan produk yang tersedia saat ini.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {merchant.products.map((product) => (
                  <div key={product.id} className="group bg-white flex flex-col md:flex-row gap-0 md:gap-6 p-2 rounded-[2rem] border border-neutral-light/50 hover:border-accent/30 hover:shadow-xl hover:shadow-accent/5 transition-all duration-300">
                    <div className="w-full md:w-2/5 h-48 md:h-auto min-h-[200px] rounded-[1.5rem] overflow-hidden flex-shrink-0 relative">
                      {product.imageUrl ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={product.imageUrl} alt={product.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                      ) : (
                        <div className="absolute inset-0 bg-gradient-to-br from-neutral-50 to-neutral-100 flex items-center justify-center border border-neutral-light/50 rounded-[1.5rem] m-2">
                    ...
                    <div className="flex-1 flex flex-col justify-between p-4 md:py-6 md:pr-6">
                      <div>
                        <h3 className="text-xl font-extrabold text-primary group-hover:text-accent transition-colors line-clamp-3 pb-1">{product.name}</h3>
